...
class EnAttenteState extends GameState {
  ...
  async handleRejoindreJeu(nsp, socket, pseudo, socket_id) {
    ...
    debug(
      "[EnAttenteState] Joining player pseudo = " +
        pseudo +
        " + player_id = " +
        player_id._id
    );
    ...
    if (this.context.nb_actif_players == this.context.nbParticipantSouhaite) {
      if (this.timeout) {
        ...
        throw new Error(
          "Timer was not set up correctly or  we are not in the correct state"
        );
      }
    }
  ...
  async createGamechat() {
    ...
    if (this.context.nsp == null) {
      debug(
        "No player joined the game thus this game context is connected to any socket"
      );
      return;
    }
    ...
    await chatGeneral.save().catch((err) => {
      debug("General chat wasn't created  error" + err);
    });
    ...
    await chatLoup.save().catch((err) => {
      debug("Chat loup chat wasn't created  error" + err);
    });

    const playerJoueurLink = await Joueur_partie_role.find({
      id_partie: this.context.partieId,
    });
    if (!playerJoueurLink) {
    ...
    playerJoueurLink.map((player) => {
      ...
      if (!playerSocket) {
        debug(
          "Player socket was not found, id of the player = " + player.id_joueur
        );
        return;
      }
    ...
    debug(
      "Chats rooms have been created for the game  : " + this.context.partieId
    );
  ...
  async setupCode() {
    ...
    debug(
      "Launched game timer, starting the game in :",
      this.context.tempsDebut * 1000 + " ms"
    );
    this.timeout = setTimeout(
      this.startGameTimer.bind(this),
      this.context.tempsDebut * 1000
    );
  }
}
